...
import cn.silently9527.download.download.SimpleDownloader;
...
public class FastDownloadApplication {
    ...
    public static void main(String[] args) throws IOException {
        ...
        System.out.println("请输入您的下载链接:");
        String fileURL = console.readLine();
        System.out.println("请输入您的下载方式(仅仅输入序号):");
        System.out.println("1：小文件下载(先下载到您的内存再输出)；\n" +
                "2：单线程下载(大文件单线程下载)\n" +
                "3：多线程下载(大文件多线程下载)");
        int downloadType= Integer.parseInt(console.readLine());
        System.out.println("请输入您的下载位置:");
        String filePath = console.readLine();


        MultiThreadDownloadProgressPrinter downloadProgressPrinter = new MultiThreadDownloadProgressPrinter(32);
        switch (downloadType){
            case 1-> {
                Downloader simpleDownloader = new SimpleDownloader();
                simpleDownloader.download(fileURL,filePath);
            }
            case 2-> {
                Downloader fileDownloader = new FileDownloader();
                fileDownloader.download(fileURL,filePath);
            }
            case 3-> {
                System.out.println("请输入您的线程数量:");
                int num = Integer.parseInt(console.readLine());
                Downloader multiThreadFileDownloader = new MultiThreadFileDownloader(num);
                multiThreadFileDownloader.download(fileURL,filePath);
            }
            default -> throw new IOException("发生了异常");
        }
